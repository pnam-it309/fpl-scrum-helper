...
<template>
  <li>
    <router-link
      ...
      class="group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-bold text-black-1 hover:text-black-2 duration-300 ease-in-out hover:bg-gray-200 dark:hover:bg-meta-4"
      @click.prevent="handleItemClick"
      :class="{
        'bg-gray-200 dark:bg-gray-200 text-black-2': sidebarStore.page === item.label
      }"
    >
      <span
        v-html="item.icon"
        :class="{ 'text-blue-500': sidebarStore.page === item.label }"
      ></span>
