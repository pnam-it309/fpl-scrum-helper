...
@Service
@Slf4j
public class ImportCompensation implements JobReportCompensationService {

    @Value("${file.upload.reportcompensation.path}")
    private String FILE_UPLOAD_REPORT_COMPENSATION_PATH;

    private Path root;

    @PostConstruct
    public void init() {
        root = Paths.get(FILE_UPLOAD_REPORT_COMPENSATION_PATH);
        try {
