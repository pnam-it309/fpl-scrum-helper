...
public class ExcelOneFileMultiSheet {

    public static void exportExcelOneFileMultiSheet(String filePath,
        ...
        for (Map.Entry<String, List<DocADProjectMemberResponse>> entry : membersByProject.entrySet()) {
            ...
            List<DocADProjectMemberResponse> membersOfProject = entry.getValue();

            String projectName = docADProjectRepository.findProjectStartTimeById(projectId).getName();

            XSSFSheet sheet = workbook.createSheet(projectName.length() > 30 ? projectName.substring(0, 30) : projectName);

            LocalDate minStartDate = membersOfProject.stream()
            ...
            for (DocADProjectMemberResponse member : membersOfProject) {
